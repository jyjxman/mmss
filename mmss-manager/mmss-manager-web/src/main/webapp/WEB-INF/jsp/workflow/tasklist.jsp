...
function disposetask(url,taskid,yycgdid){
	var sendUrl = "${baseurl}"+url+"?taskid="+taskid+"&yycgdCustom.id="+yycgdid;
	parent.opentabwindow('采购单任务',sendUrl);//打开一个新标签
}
...
var columns = [ [
 
 {
	...
	width : 60,
	formatter:function(value, row, index){
		return '<a href=javascript:disposetask("'+row.taskurl+'","'+row.taskid+'","'+row.id+'")>处理任务</a>';
	}
}]];
...
function initGrid(){
	$('#yycgdlist').datagrid({
		...
		striped : true,
		//collapsible : true,
		url : '${baseurl}/cgd/flow/tasklist_result.action',
