...
var addyycgdmx = function(){
	_confirm('您确定要采购选择的药品吗?',null,
	  function(){
		var ids = [];//定义了一个数组准备存储药品id
		var rows = $('#ypxxlist').datagrid('getSelections');//调用datagrid的getSelections方法获取选中datagrid的行
		for(var i=0;i<rows.length;i++){
			ids.push(rows[i].id);//将选 中行中的id属性值（药品id）放入ids数组
		}
		if(ids.length>0){
			$("#ypxxids").val(ids.join(','));//将ids数组中的数据以逗号分隔的串方式格式化，存储到了ypxxids的元素中
			alert($("#ypxxids").val());
...
function addyycgdmx_callback(data) {
	...
	function initGrid() {
		$('#ypxxlist').datagrid({
			...
			striped : true,
			//collapsible : true,
			url : '${baseurl}/cgd/addyycgdmx_ypxxlist.action',
			...
			//sortOrder : 'desc',
			//remoteSort : false,
			idField : 'id',//datagrid的唯一标识 ，为json中key，如果此列不唯一影响 调用datagrid的getSelections方法
